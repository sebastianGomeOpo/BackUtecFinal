...
# In-memory storage for conversation activity (replaces MongoDB collections)
_conversation_activity: Dict[str, Dict] = {}
_followup_logs: List[Dict] = []
_pending_discounts: Dict[str, Dict] = {}
...
class FollowUpMonitor:
    ...
    async def reset_timer(self, conversation_id: str):
        ...
        global _conversation_activity

        if conversation_id not in _conversation_activity:
            _conversation_activity[conversation_id] = {}

        _conversation_activity[conversation_id].update({
    ...
    async def _monitor_conversation(self, conversation_id: str):
        ...
        while True:
            try:
                ...
                activity = _conversation_activity.get(conversation_id, {})
                ...
                # Check if enough time has passed since last assistant response
                ...
                if time_since_message.total_seconds() >= self.INACTIVITY_MIN_SECONDS:
                    ...
                    if followup_count < self.MAX_FOLLOWUPS:
                        ...
                        _conversation_activity[conversation_id]["followup_count"] = followup_count + 1
                        _conversation_activity[conversation_id]["last_followup"] = datetime.utcnow()
                    else:
    ...
    async def _get_conversation_context(self, conversation_id: str) -> Dict:
        ...
        try:
            ...
            # Get viewed products from activity
            activity = _conversation_activity.get(conversation_id, {})
            has_viewed_products = activity.get("has_viewed_products", False)
    ...
    async def _send_followup(self, conversation_id: str, followup_index: int):
        ...
        global _followup_logs, _pending_discounts
        ...
        if discount_percent > 0:
            _pending_discounts[conversation_id] = {
                "discount_percent": discount_percent,
